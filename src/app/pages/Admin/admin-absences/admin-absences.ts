...
export class AdminAbsences implements OnInit, OnDestroy {
  ...
  columns: (string | dxDataGridColumn<any, any>)[] = [
    { dataField: 'employeeName', caption: 'Employee', allowFiltering: true },
    ...
    { dataField: 'numberOfDays', caption: 'Days' },
    {
      ...
      calculateCellValue: (row: any) => {
        return row.employeeRole === 'TeamLead' ? 'Team Lead' : (row.employeeRole ?? '');
      }
    },
    {
      caption: 'Status',
    ...
    },
    {
      ...
      cellTemplate: (container: any, options: any) => {
        ...
        if (data.status !== 'PendingAdministration') return;
  ...
  getStatusLabel(status: string): string {
    const labels: Record<string, string> = {
      ...
      'PendingAdministration': 'Pending Approval',
      'Accepted': 'Accepted',
      'RejectedTeamLead': 'Rejected by TL',

...
export class PublicHolidays implements OnInit {
  ...
  columns: dxDataGridColumn[] = [
    ...
    { dataField: 'name', caption: 'Holiday Name' },
    { dataField: 'nameAr', caption: 'الاسم', width: 150 },
    {
      ...
      cellTemplate: (container: any, options: any) => {
        ...
        badge.textContent = options.data.isRecurring ? 'Yearly' : 'Once';
  ...
  save(validationGroup: any): void {
    ...
    if (this.isEditMode && this.form.id) {
      this.http.put<ApiResponse<string>>(
        `${environment.apiUrl}/public-holidays/${this.form.id}`, this.form
      ).subscribe({
        next: (res) => {
          if (res.isSuccess) {
      ...
      this.http.post<ApiResponse<string>>(
        `${environment.apiUrl}/public-holidays`, this.form
      ).subscribe({
        next: (res) => {
          if (res.isSuccess) {
  ...
  confirmDelete(): void {
    ...
    this.http.delete<ApiResponse<string>>(
      `${environment.apiUrl}/public-holidays/${this.holidayToDelete.id}`
    ).subscribe({
      next: (res) => {
        if (res.isSuccess) {
  ...
  seedHolidays(): void {
    ...
    this.http.post<ApiResponse<string>>(
      `${environment.apiUrl}/public-holidays/seed/${this.selectedYear}`, {}
    ).subscribe({
      next: (res) => {
        if (res.isSuccess) {
